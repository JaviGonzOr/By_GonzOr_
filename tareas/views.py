...
from .forms import TareaForm, Proveedor, ProductoForm, VisitasForm
from .models import Tarea, Proveedor, Producto, ImagenTarea, Visitas
        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                user = User.objects.create_user(
                    username=request.POST['username'], password=request.POST['password1'], last_name=request.POST['last_name'])
                user.save()
                login(request, user)
                return render(request, 'home.html', {
